/**
 * cloud-backup.js - KHub optional Cloud Backup Module
 *
...
(function () {
  ...
  function writeKeys(savedKeys) {
    Object.keys(savedKeys || {}).forEach(function (k) { localStorage.setItem(k, savedKeys[k]); });
  }

  function markSaved(appId, iso) { localStorage.setItem(markerKey(appId), iso || new Date().toISOString()); }
  ...
  function ensureReady() {
    if (!window.KHub || !KHub.Firebase || !KHub.Firebase.db) return Promise.reject(new Error('Firebase not ready'));
    return null;
  }
  ...
  window.KHub.CloudBackup = {
    save: function (appId, exactKeys, scanPrefix) {
      ...
      return Promise.all([deviceRef(appId).set(payload), latestRef(appId).set(payload)]).then(function () {
        markSaved(appId, iso);
        return iso;
      });
    },
    ...
    lastSaved: function (appId) { return localStorage.getItem(markerKey(appId)); },
