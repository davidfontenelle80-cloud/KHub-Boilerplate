...
2. **Read the project status doc — `PROJECT_STATUS.md`.** In this reference-only repo that
   role is served by `README.md` + `CLAUDE.md`; a real KHub project has its own
   `PROJECT_STATUS.md`. Read whichever exists.
...
5. **Implement** the current task. Stay inside the "files expected to change" set; never
   ...
   Supervisor first.
6. **Verify.** Run the checks the task calls for (for KHub apps: `node scripts/khub-check.mjs .`
   and the ship check in `CLAUDE.md`; for docs: confirm links resolve and nothing existing
   broke). Record exactly what you verified in `ACTIVE_TASK.md`.
...
**Only the Supervisor (David) writes in that section.** Workers implement, test, verify,
commit, and deploy — but they do **not** approve their own work. See `CONTRIBUTING_AI.md`
for the full Supervisor-vs-Worker responsibility split.

---

## Copy-paste starting point for a fresh ACTIVE_TASK.md

If you are starting a brand-new task and need a blank `ACTIVE_TASK.md`, copy the template
from the portable starter kit: `_ai-session-continuity-standard/.ai/ACTIVE_TASK.md`.
