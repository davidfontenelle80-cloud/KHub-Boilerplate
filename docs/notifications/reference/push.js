(function () {
  ...
  function cfg() {
    var external = window.KHUB_PUSH_CONFIG || {};
    return {
      ...
      appId: String(external.appId || DEFAULT_CONFIG.appId),
  ...
  function requireConfigured() {
    if (!isConfigured()) {
      throw new Error(
        'Background push is not configured. Set KHUB_PUSH_CONFIG.workerUrl and vapidPublicKey.'
      );
    }
    return cfg();
  }
  ...
  function subscriptionUsesKey(subscription, vapidPublicKey) {
    ...
    return (
      ...
      String(vapidPublicKey || '').replace(/=+$/g, '')
    );
  }

  function getSubscriptionId() {
    try {
      return localStorage.getItem('ministryPushSubscriptionId') || '';
    } catch (e) {
      return '';
    }
  }
  ...
  function pushErrorResult(action, err) {
    ...
    console.warn('[KHubPush] ' + action + ' handled failure:', message, err || '');
  ...
  function subscribe() {
    ...
    if (!('PushManager' in window))
      return Promise.reject(new Error('PushManager is not supported.'));
  ...
  function syncReminder(sourceType, sourceId, title, body, fireAt) {
    ...
    try {
      ...
      return Promise.resolve(pushErrorResult('reminder:sync', err));
    }
  ...
  function clearReminder(sourceType, sourceId) {
    ...
    try {
      ...
      return Promise.resolve(pushErrorResult('reminder:clear', err));
    }
  ...
  function sendTestPush() {
    ...
    try {
      ...
      return Promise.resolve(pushErrorResult('test-push', err));
    }
    return subscribe()
